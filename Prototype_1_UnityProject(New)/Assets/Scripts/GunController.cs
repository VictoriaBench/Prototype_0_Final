...
public class GunController : MonoBehaviour
{
    ...
    public void HitByEnemy() //If the enemy hits either the player hand or the player centre
    {
        ...
        //Play player sound
        FindObjectOfType<SoundManager>().Play("PlayerHurt"); //find name of sound
        lifeBar.GetComponent<LifeBar>().LoseHealth(0.5f);


        if (lifeBar.GetComponent<LifeBar>().currentValue <= 0)
        {
    ...
    {
        lifeBar.GetComponent<LifeBar>().GainHealth(0.1f);
    }
