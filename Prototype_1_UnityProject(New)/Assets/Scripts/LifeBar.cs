...
public class LifeBar : MonoBehaviour
{
   ...
   // public Text displayText;
    ...
    {
        StartCoroutine(MoveLifeBar(amount));
    }
    public void GainHealth(float amount)
    {
        // StartCoroutine(MoveLifeBarUp(amount));
        CurrentValue += amount;
    }


    IEnumerator MoveLifeBar(float amount)
    {
        float value = CurrentValue; 
        while(CurrentValue > value - amount)
        {
            CurrentValue -= 0.0086f;
            yield return null;
        }
    ...
    IEnumerator MoveLifeBarUp(float amount)
    {
        float value = CurrentValue;
        while (CurrentValue < value + amount)
        {
